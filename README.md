...
- Row Level Security במסד הנתונים, וקובצי PDF ב־bucket פרטי שנגיש רק למשתמשי HUJI
  מחוברים, דרך קישורים זמניים. מבקרים לא מחוברים מופנים לארכיון הבחינות של האוניברסיטה.
- טעינה לפי קורס ובעמודים: האתר אינו מוריד את כל המאגר בפתיחה, ואין חיתוך שקט בגבול
  1000 השורות של Supabase.
...
2. פתחו את SQL Editor והריצו לפי הסדר את הקבצים
   ...
   `supabase/migrations/202609260001_huji_only_auth.sql` ולאחריו
   `supabase/migrations/202609270001_hardening_and_scale.sql`.
...
רמזים חדשים נשמרים בסטטוס `pending` ואינם מוצגים עד שמנחה מאשר אותם. כדי להגדיר
מנחה, הריצו פעם אחת ב־SQL Editor:
...
מנחה מחובר יכול לקרוא את התור ולאשר או להסתיר רמז (למשל מתוך קונסולת הדפדפן או
מכלי ניהול עתידי):
...
שורות עם `nature = "archive"` הן מצייני מקום למבחן שעוד לא חולץ. הן אינן נספרות
כשאלות, ו־`fetch_range.py` וגם `npm run ingest:course` מחלצים אותן אוטומטית בהרצה
רגילה, בלי `--force-extract`, כך שמבחנים שכבר חולצו אינם נוגעים. עבור 80181:
