...
function normaliseBox(item, page) {
  ...
  if (unit.includes("percent") || unit === "%" || (largest > 1.2 && largest <= 100)) {
    x /= 100;
    y /= 100;
    w /= 100;
    h /= 100;
...
export function pagePrompt(exam, page, total, topics = {}, lastLabel = "") {
  ...
  return [
    ...
    "For every item, draw bbox={x,y,w,h} around the ORIGINAL PRINTED QUESTION on this page.",
    "Coordinates should be 0–1 page fractions with origin at the top-left. Pixel or percent coordinates are accepted but fractions are preferred.",
